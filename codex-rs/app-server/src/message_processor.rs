...
impl MessageProcessor {
    ...
    async fn handle_client_request(
        ...
        match codex_request {
            ...
            ClientRequest::AgentWorkspaceFileWrite { request_id, params } => {
                self.handle_agent_workspace_file_write(
                    ConnectionRequestId {
                        connection_id,
                        request_id,
                    },
    ...
    async fn handle_agent_workspace_file_write(
        &self,
        request_id: ConnectionRequestId,
        params: AgentWorkspaceFileWriteParams,
    ) {
        match self.config_api.agent_workspace_file_write(params).await {
            Ok(response) => self.outgoing.send_response(request_id, response).await,
            Err(error) => self.outgoing.send_error(request_id, error).await,
        }
    }
